...
void* Router::listen_on_port(void* thread_id){
  ...
  cout<<endl<<"Starting to listen on port " << nbr_tbl.at(*tid).RxP<<"......\n";
  while(1){
  	Packet* rxed_p = rxing_ports[*tid]->receivePacket();
  }
}
...
void Router::open_disc_tx_channels(){
	if(num_neighbors == 0){
 	...
 	disc_txing_ports = new SendingPort*[num_neighbors];
	...
	for(int i=0; i<num_neighbors; i++){
  	...
  	disc_txing_ports[i] = new SendingPort();
...
void* Router::tx_disc_message_on_port(void* thread_id){
	int* tid = (int *)(thread_id);
  cout<<endl<<"Sending disc message on port " << nbr_tbl.at(*tid).TxP<<"......\n";
  //send disc message
}
